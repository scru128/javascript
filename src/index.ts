...
export class Scru128Generator {
  ...
  /**
   * Generates a new SCRU128 ID object from the current `timestamp`,
   * guaranteeing that the returned ID is greater than the immediately preceding
   * one generated by the same generator.
   *
   * This method returns monotonically increasing IDs unless the up-to-date
   * `timestamp` is significantly (by ten seconds or more) smaller than the one
   * embedded in the immediately preceding ID. If such a significant clock
   ...
   */
  generateMonotonic(): Scru128Id | undefined {
    return this.generateCoreMonotonic(Date.now());
  }
   ...
   * detected, this method resets the generator state and returns a new ID based
   * on the given argument.
   *
   * @throws RangeError if the argument is not a 48-bit positive integer.
   */
  generateCore(timestamp: number): Scru128Id {
    let value = this.generateCoreMonotonic(timestamp);
    if (value === undefined) {
      // reset state and resume
      this.timestamp = 0;
      this.tsCounterHi = 0;
      value = this.generateCoreMonotonic(timestamp) as Scru128Id;
   ...
   * that the returned ID is greater than the immediately preceding one
   ...
   * @throws RangeError if the argument is not a 48-bit positive integer.
   */
  generateCoreMonotonic(timestamp: number): Scru128Id | undefined {
   ...
   *   ten seconds or more.
   *
   * @deprecated Use {@link generateMonotonic | generateMonotonic()} to
